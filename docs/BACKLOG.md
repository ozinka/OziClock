...
| --- | --- | --- | --- | --- | --- |
| BL-058 | P3 | In Progress | Plan events | Add personal calendar events | EVT-01–05 establish typed validated mutations, Week rendering, selection, and a shared timed/all-day create/edit modal. Next add confirmed deletion, overlap lanes, calendar date picking, alert offsets and Does not repeat/Daily/Weekly/Monthly/Yearly recurrence. Exclude participants, invitations, shared calendars and network sync. Preserve a series model for future Custom rules and per-occurrence exceptions. |
