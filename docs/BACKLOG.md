...
| --- | --- | --- | --- | --- | --- |
| BL-058 | P3 | Candidate | Plan events | Add personal calendar events | Replace the Plan placeholder with local events containing title, start/end or All day, time zone, optional location/notes/link/color, alert offsets, and Does not repeat/Daily/Weekly/Monthly/Yearly recurrence. Render timed events as duration blocks in Week view and all-day events in a dedicated row. Exclude participants, invitations, shared calendars and network sync. Design recurrence as a series so Custom rules and per-occurrence exceptions can be added later without changing event identity. |
