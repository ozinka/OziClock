...
| --- | --- | --- | --- | --- | --- |
| BL-058 | P3 | In Progress | Plan events | Add personal calendar events | EVT-01–08 establish typed validated mutations, Week rendering, selection, a shared timed/all-day editor, confirmed deletion, overlap lanes, calendar date picking, 15-minute time controls, and persisted Does not repeat/Daily/Weekly/Monthly/Yearly series expansion. Next add alert offsets. Exclude participants, invitations, shared calendars and network sync. Preserve the series model for future Custom rules and per-occurrence exceptions. |
...
| BL-055 | P1 | In Progress | Plan | Complete the interactive weekly schedule | PLN-01–03 and PLN-06–07 provide live week dates, navigation, current-day/time emphasis, toggleable cells/reminders, contextual selection details, direct reminder editing, and deterministic overlap lanes. Verify selection, current-time placement and lane geometry at minimum/expanded sizes; Month/Year integration remains a later slice and empty-cell Event creation remains reserved for BL-058. |
