...
| --- | --- | --- | --- | --- | --- |
| BL-058 | P3 | In Progress | Plan events | Add personal calendar events | EVT-01–11 establish typed validated mutations, Week rendering, selection, a shared timed/all-day editor, confirmed deletion, overlap lanes, date/time controls, persisted recurrence, alert offsets, durable local attention and restart recovery. Visual verification remains; Custom recurrence/alerts and per-occurrence exceptions stay later. Exclude participants, invitations, shared calendars and network sync. |
