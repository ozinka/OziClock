...
| BL-042 | P2 | In Progress | Delivery | Add native operating-system notifications and sound | NTF-01 uses a cross-platform notification adapter, macOS `UNUserNotificationCenter`, non-fatal failure handling and a platform sound for newly delivered Planner items. Local attention remains the durable fallback; permission/settings UI and notification actions remain. |
