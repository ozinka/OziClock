...
- **PLN-03:** Clicking an empty hourly cell selects it without leaving Plan, and clicking it again clears the selection. Clicking a reminder marker selects or clears that reminder. Double-clicking a reminder marker opens the shared Reminder editor. Double-clicking an empty cell remains reserved for creating a personal Event once Events are implemented and must not create a Reminder implicitly. The current-day column uses a quiet lighter surface across its full schedule range, and a contrasting line with a local-time label indicates the current time while it is within the visible 09:00–18:00 range. Explicit cell and marker selections remain visually stronger. Changing the Plan range or view clears stale selection context, as does a refresh that removes the selected reminder from the visible week.
