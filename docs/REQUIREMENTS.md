...
- **SET-11:** Settings offers an opt-in Sync profile in a user-chosen folder managed by an installed Google Drive, OneDrive, Dropbox, or equivalent desktop client. Each device independently selects whether to sync Planner data (Events, Tasks, and Reminders), Clocks, and Appearance. The profile is one versioned `oziclock-sync.json` document; it contains only selected portable groups, never device settings such as window placement, launch-at-login, taskbar visibility, always-on-top state, system permissions, audio output, active timers, stopwatch state, alarms, local attention, or delivery receipts. The UI supports continuous bidirectional synchronization plus previewed one-time Send to profile and Receive from profile actions. Sync never delays local saves, scheduling, or delivery. When both sides changed the same entity or field since their last common revision, it shows a conflict choice instead of silently discarding either value; changes to independent entities or fields merge. Missing, invalid, unavailable, or unwritable profiles remain non-fatal and retain the last valid local state.
