...
- **MODE-06A:** The outline and separators share a nominal one-logical-pixel thickness scaled with the clock UI, rounded to at least one whole physical pixel. Adjacent columns share pixel-aligned boundaries. Reserve one eighth of the scaled corner radius, rounded up to a physical pixel, at each outer end to provide modest clearance beside the edge digits (0 pixels at radius 0, 1 at radius 8, and 2 at radii 12/15.5 at 100% UI/display scale). Only the first and last columns gain this space; clock, ruler, lens, and slider widths stay aligned and horizontal geometry remains unchanged between display modes. Ruler and lens labels are centered across the full width of their columns, including the outer padding: with 2 pixels per end, the first/last labels move 1 pixel toward the outer edge relative to the original text area; a single column stays centered between both padded ends. Slint owns the outer border and rounded child clipping.
...
- **CLK-06A:** In the Settings clock list, emphasize the main clock label with bold black text in Light theme and bold white text in Dark theme, including its dragged row. Other clock labels retain the regular theme text color. The clock strip keeps its own surface styling.
...
- **CLK-08D:** Switching to another clock clears the time-zone search and any no-results message, restores the complete time-zone list, and selects the newly edited clock's configured zone. This reset must not change either clock's configured zone.
...
- **RUL-03A:** Only the main-clock ruler displays `24` at its final label; all other rulers wrap from `23` to `0` after applying their current time-zone offset.
- **RUL-04A:** The ruler surface has a one-pixel black outer border and one-pixel black joins between 99-pixel clock columns.
...
- **RUL-07A:** The bottom time-slider panel follows the persisted application Light/Dark theme live, including its background, hour labels and slider control. Clock/ruler colors, the configured frame color and the selected time remain unchanged by this theme update.
...
- **SET-03:** Save one local application document atomically at the fixed per-user path defined by SET-12. Create missing directories and initial settings on first launch. Recover with safe defaults from missing, invalid, or older configuration, and migrate legacy settings without overwriting an existing document at the new location. Local Sync configuration and revision/conflict state belong to the document's `Sync` section and are never exported in the shared profile.
...
- **SET-08:** Planner has an independent Light/Dark theme, applied live to Planner, its editors, and every attention window. Existing and new configurations default to Dark; Calendar's own theme remains independent.
- **SET-09:** Planner can follow the main clock accent (including its existing 40% brightness adjustment) or use a custom #RRGGBB accent. Default to following the main clock; retain the custom value while following. Apply changes live to open and hidden windows without losing editor drafts. Custom colors use contrasting foregrounds. Clicking an enabled Planner color swatch opens the shared color picker with live preview; Cancel restores its opening color. This applies to the custom accent and all five type colors, alongside hex input.
- **SET-10:** Planner Settings persists a default-enabled `Use accent color` switch. When enabled, use the Planner accent for all five attention types and Event/Reminder/Task markers and summaries in Plan's calendar views. When disabled, enable separate persisted Alarm, Timer, Reminder, Event, and Task colors. Keep those values while the override is enabled. Missing appearance fields migrate independently to Dark, main-clock accent, enabled override, teal Alarm/Timer/Reminder, gold Event, and muted teal Task. Invalid color text does not replace the last valid color; malformed stored colors use safe defaults. Appearance changes preview immediately and persist with Settings OK or close, alongside alert sound duration.
- **SET-11:** Settings offers an opt-in Sync profile in a user-chosen folder managed by an installed Google Drive, OneDrive, Dropbox, or equivalent desktop client. Each device independently selects whether to sync Planner data (Events, Tasks, and Reminders), Clocks, and Appearance. A native folder chooser selects the location; folder and group choices persist immediately. One `Sync now` action creates the first `oziclock-sync.json` copy for an empty folder, then sends local changes or gets a newer cloud copy. The profile contains only selected portable groups, never device settings such as window placement, launch-at-login, taskbar visibility, always-on-top state, system permissions, audio output, active timers, stopwatch state, alarms, local attention, or delivery receipts. An optional automatic mode publishes selected local changes without delaying the local save and checks the provider folder at a persisted positive interval in minutes, defaulting to 30. When both sides changed the same entity or field since their last common revision, it shows a conflict choice instead of silently discarding either value; changes to independent entities or fields merge. Missing, invalid, unavailable, or unwritable profiles remain non-fatal and retain the last valid local state.
- **SET-12:** The local file has a fixed per-user location: `%USERPROFILE%\.oziclock\settings.json` on Windows, `$XDG_CONFIG_HOME/oziclock/settings.json` on Linux (default `~/.config/oziclock/settings.json`; ignore empty or relative XDG values), and `~/Library/Application Support/OziClock/settings.json` on macOS. Do not offer a local-folder selector or maintain a location bootstrap or alternate active recovery store. Settings calls the provider-profile section Sync and retains only SET-11 controls there. On first upgrade, import the old active custom file selected by the legacy bootstrap, otherwise the former default file beside the executable (or the older file beside `OziClock.app` on macOS). Import valid legacy local Sync state into the same document. An existing new-location document takes precedence; on macOS the pre-migration default recovery copy may be replaced by the valid custom document. Persist a schema migration marker in the document so later launches ignore legacy files. Retain old source files without updating them; subsequent saves use only the fixed location and report failures without switching paths.
- **SET-13:** General Settings owns one persisted Light/Dark application theme. Apply it live to Settings, Calendar, Planner, Planner editors, every attention window, About, the context menu, the bottom ruler time slider, and future auxiliary windows; remove per-window theme controls. Existing settings migrate to the prior Calendar theme value, preserving the visible theme on first launch after upgrade. The main clock’s surface controls remain independent. Keep current accent and type-color behavior, and preserve readable contrast in both themes.
...
- **ALM-03:** Saved alarm cards align with the section heading's left edge. Once/Weekly labels use the Planner accent; one-time dates remain below the title and both alarm types show the time right-aligned before the toggle. When following the main clock (the default), Planner uses a 40% brighter variant of the shared clock-derived accent, both initially and during live updates; SET-09 permits an independent custom accent. The alarm editor weekday selectors use 14px text without button backgrounds, retaining 42px by 30px hit targets. Selected weekdays are blue and weekends red; unselected days and all days in Once mode are gray. Once disables day selection without clearing it.

- **UI-08:** Clock-following Planner and Calendar accents update immediately when the main clock color or main clock selection is applied in Settings, including live color preview. Hidden windows retain the updated accent for their next opening; no window recreation or restart is required.

- **UI-07:** Planner can be dragged from non-interactive background areas, including the full header and empty sidebar. Interactive controls (including close, navigation, text editors, and scroll areas) retain their pointer behavior; the resize edge remains available. Selecting Planner from the clock context menu shows, restores, positions it beside the clock within the active monitor work area, raises, and focuses the existing window without hiding/recreating it or resetting its selected section and drafts. Planner is an auxiliary window and never creates a separate Windows taskbar entry.
- **UI-07A:** The clock context menu also offers Timer, Stopwatch and Reminder. Each opens or reactivates the existing Planner with Timers, Stopwatch or Reminders selected, using the same section-entry behavior as its sidebar. Open Planner retains the current section. An open modal editor remains visible with its draft intact instead of switching sections underneath it. Every menu item has a leading outline icon in the Planner sidebar style, sharing the same icons for Planner time tools. A horizontal divider follows Reminder; the existing divider before Exit remains.
...
- **UI-02 (Legacy):** Position Edit, Settings, and Calendar near the clock strip, preferring below it, falling back above, and constraining them to the working area. Settings and Calendar keep a five-logical-pixel gap from the clock strip on either side.
...
- **UI-09:** Settings clock deletion opens a dimmed in-window modal centered on both axes. Its fixed 462 × 190 logical-pixel card remains fully within the minimum 710 × 672 logical-pixel Settings window, blocks clicks behind it, and allows explicit Cancel, Remove, or backdrop dismissal. Settings has a fixed 710-pixel width and a vertically resizable height of at least 672 pixels; the height persists between launches. Windows exposes six-logical-pixel top and bottom resize targets inside the frameless window, excluding the rounded corners, with a vertical resize cursor and native vertical dragging. Header movement, controls and modal input blocking retain their behavior; other platforms retain their native resizing.
- **UI-10:** Every shared segmented time editor (Alarm, Timer, Reminder, including modal editors) supports the mouse wheel over a segment: upward increases by one, downward decreases by one, focuses/selects that segment, and consumes vertical scrolling without moving the surrounding view. Horizontal-only scrolling does not change values. Wheel, keyboard Up/Down, and shared arrow buttons use current text and identical rules: hours wrap 23 ↔ 0, minutes/seconds wrap 59 ↔ 0, and timer days clamp to 0–999. Wrapping never changes neighboring segments or the date. Direct entry and Tab navigation remain available.
...
- **CAL-08:** At four or more clocks the calendar may visually align with the strip. With one to three clocks it retains a usable minimum width, centers below the strip, and keeps rounded upper corners. Calendar keeps a five-logical-pixel gap from the clock strip, and its height must not grow merely because clock UI scale or clock count increases.
- **CAL-11:** Calendar Month and Week views show a compact row of colored indicators for every local date with calendar-relevant Planner items; Year remains a clean navigation overview without dots. Show at most one dot per type: Event (including all-day, multi-day, and recurring occurrences), Reminder, Task with a due date, and Alarm occurrence. Dots use their configured type colors, convey no count or priority, and remain legible in both Calendar themes. Timers and Stopwatch never produce dots because they have no calendar date. A selected date has a compact read-only day preview listing the matching items' type, title and local time, or a clear empty state. Indicators and the preview derive from existing Planner projections in the main clock's time zone; Calendar owns neither item storage nor scheduling.
- **CAL-12:** Calendar provides a visible `Open in Planner` action for the selected date; double-clicking a date in Month or Week invokes the same action. Activating it raises the existing Planner or opens it, selects Plan, and shows the selected date: Week shows its containing week and focuses that date; Month shows its containing month and selects that date; Year opens its containing Month. Calendar hides only after this handoff completes. Planner remains visible after it loses focus. If a Planner editor with an unsaved draft is open, the action raises that editor without changing the Planner range or discarding the draft; the Calendar remains available for a later handoff.
- **CAL-13:** Calendar replaces its direct theme toggle with four compact, persisted indicator filters: Events, Reminders, Tasks, and Alarms. Each control presents its type color, label, and enabled state. It affects only that type’s dot in Month and Week; it never hides the matching item from the selected-day preview, Planner, scheduling, or alerts. All four filters default to enabled, remain available in either application theme, and Year continues to show no dots.
...
- **ALM-01:** Alarm creation uses the same single-frame segmented time editor as timers, with H:M only and initial time 07:00. The active segment is highlighted; shared arrow symbols and keyboard Up/Down adjust the current text within 0–23 hours and 0–59 minutes with bidirectional wrapping, as specified in UI-10. Tab and direct entry remain available. Both one-time and weekly alarms store normalized HH:MM values; invalid or incomplete times are rejected and midnight is valid.

- **TMR-01:** Create timer durations using a single framed D:H:M:S control. Defaults are 0:0:5:0. Each segment supports direct text entry, with the active segment highlighted; one shared pair of unfilled arrow symbols adjusts the active segment. Keyboard Up/Down and the mouse wheel provide the same adjustment and Tab moves between segments.
- **TMR-02:** Days range from 0 to 999, hours from 0 to 23, and minutes/seconds from 0 to 59. Hours, minutes, and seconds wrap at their limits in both directions without carrying into other segments; days stop at 0 and 999. Invalid or all-zero durations do not create a timer. Adjustments use the current text, never stale stored values.
...
- **REM-11:** The date remains directly editable and has an adjacent calendar icon. Activating it opens a Monday-first six-week month picker focused on the entered date (or today when invalid); past dates are disabled, today is outlined, the selected date uses the accent, adjacent-month days remain visible, and choosing a date updates the field and closes the picker. The shared Reminder/Task/Event picker aligns dates in six horizontal rows and seven columns centered under weekday headings. All date-picker buttons reuse a drawn calendar outline rather than a font-dependent glyph.
...
- **PLN-03:** Clicking an empty hourly cell selects it without leaving Plan, and clicking it again clears the selection. Clicking a reminder marker selects or clears that reminder; selection uses a contrasting outline while preserving the marker's accent fill. Double-clicking a reminder marker opens the shared Reminder editor. Double-clicking an empty cell creates a personal Event. The current-day column uses a quiet lighter surface across its full schedule range, and a contrasting line with a local-time label indicates the current time. Explicit cell and marker selections remain visually stronger. Changing the Plan range or view clears stale selection context, as does a refresh that removes the selected reminder from the visible week.
...
- **PLN-08:** Month renders a six-week calendar with daily Event and scheduled-task counts and red Saturday/Sunday column headings matching Week; Year renders twelve month summaries. Each Year card shows Event and Task counts only when that count is greater than zero; a task-only summary uses the first summary row, and empty months show only their name and current-month emphasis. Both support previous/current/next navigation, preserve their visible period, highlight today, and allow a Year month to open in Month view.
- **PLN-09:** Week uses a vertically scrollable 00:00–24:00 timeline with a fixed 60-pixel hour height and an 18-pixel trailing inset. Weekday headings and the all-day Event row stay fixed above the scrolling hourly grid, with aligned day columns. The scroll extent is exactly 24 hours plus the trailing inset; header height does not add blank space at the bottom. Increasing Planner height reveals more hours instead of stretching rows. On initial opening, the viewport starts near the current local time, and the current-time line remains positioned by its absolute minute of day.
- **PLN-10:** Open tasks with a due time appear in the corresponding Week cell. Click selects or deselects the task without changing its status; double-click opens the shared task editor.

## Personal Events

- **EVT-01:** Event mutations pass through typed application commands. Adding rejects duplicate identifiers, blank titles and invalid time ranges; editing preserves identity and rejects blank titles or invalid ranges; deleting removes only the requested event.
- **EVT-02:** A timed event requires valid RFC 3339 start and end instants, a non-empty source time-zone identifier, and an end strictly after its start. An all-day event requires valid local dates and permits a one-day range where start and end are equal.
- **EVT-03:** Week view renders stored timed events as duration-sensitive amber blocks in the main clock's time zone and clips their visible portion to 09:00–18:00. All-day events occupy a dedicated row beneath the weekday headings, including every visible day of a multi-day range; both the headings and this row remain visible during timeline scrolling. Event selection is independent from Reminder selection and exposes the event title, date, and time in the shared context panel.
...
- **EVT-12:** Event and task alert menus provide a validated Custom offset from 1 minute through 7 days in addition to common presets.
- **NTF-01:** Newly delivered alarms, timers, reminders, Events and scheduled tasks request an operating-system notification and independently request the same synthesized OziClock chime on Windows, macOS and Linux. General Settings persist one sound duration for all Planner deliveries: Off, 1, 5, 10, 20, or 30 seconds; 20 seconds is the default and old or invalid values migrate to it. The 0.8-second chime repeats with a 0.3-second pause until the selected duration ends. Dismiss, Snooze, Restart, disabling sound, or changing duration stops current playback within one polling interval. Playback uses the current default audio output, requires no system sound file, external player or audio decoder, runs outside the UI thread, and releases the audio device afterward. One application-owned worker allows one pending signal; further requests while that slot is occupied coalesce without dropping local attention cards. Device-open, stream and playback-timeout errors are diagnostic and non-fatal; subsequent signals retry with a fresh output stream. Failure or denied notification permission is non-fatal and the durable local attention card remains the authoritative fallback.
