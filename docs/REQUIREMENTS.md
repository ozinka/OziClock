...
- **MODE-06A:** The outline and separators share a nominal one-logical-pixel thickness scaled with the clock UI, rounded to at least one whole physical pixel. Adjacent columns share pixel-aligned boundaries. Reserve one quarter of the scaled corner radius, rounded up to a physical pixel, at each outer end to provide modest clearance beside the edge digits (0 pixels at radius 0, 2 at radius 8, and 4 at radius 15.5 at 100% UI/display scale). Only the first and last columns gain this space; clock, ruler, lens, and slider widths stay aligned and horizontal geometry remains unchanged between display modes. Slint owns the outer border and rounded child clipping.
...
- **SET-13:** General Settings owns one persisted Light/Dark application theme. Apply it live to Settings, Calendar, Planner, Planner editors, every attention window, About, and future auxiliary windows; remove per-window theme controls. Existing settings migrate to the prior Calendar theme value, preserving the visible theme on first launch after upgrade. The main clock’s surface controls remain independent. Keep current accent and type-color behavior, and preserve readable contrast in both themes.
