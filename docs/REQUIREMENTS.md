...
- **CAL-11:** Calendar Month, Week, and Year views show one unobtrusive event indicator for every local date containing one or more personal Events. The indicator represents Events only, including all-day, multi-day, and recurring occurrences; it does not encode count, type, priority, tasks, or reminders. A selected date has a compact read-only day preview listing that date's Event titles and local times, or a clear empty state. Indicators and the preview derive from Planner's existing Event projections in the main clock's time zone; Calendar owns neither event storage nor scheduling.
- **CAL-12:** Calendar provides a visible `Open in Planner` action for the selected date. Activating it raises the existing Planner or opens it, selects Plan, and shows the selected date: Week shows its containing week and focuses that date; Month shows its containing month and selects that date; Year opens its containing Month. Calendar hides only after this handoff completes. Planner remains visible after it loses focus. If a Planner editor with an unsaved draft is open, the action raises that editor without changing the Planner range or discarding the draft; the Calendar remains available for a later handoff.
