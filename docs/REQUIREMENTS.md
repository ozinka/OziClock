...
- **SET-03:** Save settings atomically in `settings.json` beside the executable on Windows and Linux, and under `~/Library/Application Support/OziClock` on macOS, unless the user selects a custom folder through SET-12. Recover with safe defaults from missing, invalid, or older configuration, and migrate a legacy macOS settings file found beside `OziClock.app`.
...
- **SET-12:** Settings has a Storage section that shows the active local `settings.json` folder and accepts an existing folder path. `Move data here` atomically writes the complete current document to an empty selected folder, records that folder only in a small local bootstrap file beside the platform-default store, and retains an updated platform-default recovery copy. `Use default` atomically restores the default store and clears the bootstrap. A missing, invalid, or unwritable custom folder falls back to the recovery copy without blocking local persistence. This local-location feature does not synchronize data; provider-managed sharing is SET-11.
