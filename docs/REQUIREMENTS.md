...
- **PLN-09:** Week uses a vertically scrollable 00:00–24:00 timeline with a fixed 60-pixel hour height and only a compact 12-pixel trailing inset. Increasing Planner height reveals more hours instead of stretching rows. On initial opening, the viewport starts near the current local time, and the current-time line remains positioned by its absolute minute of day.
