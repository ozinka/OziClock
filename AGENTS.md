...
- Keep related edits in one coherent change and avoid unrelated improvements. Read targeted files and relevant log excerpts. Reuse completed checks when the checked state has not changed; repeat or broaden them only for changed code, failures, or unresolved risks. During an in-progress slice, run the cheapest targeted check that can falsify the change; reserve the full validation gate for a completed functional slice or handoff.
...
Once the Rust workspace exists, use `cargo fmt --check` and focused tests while iterating. Before handing off a completed functional slice, run `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test --workspace`; broaden earlier only when the changed boundary or a failure warrants it. Time-based tests use fake UTC and monotonic clocks; visual changes retain or update applicable reviewed golden images rather than weakening comparisons.
...
After every completed source-code slice, run the required validation and build the debug application for the user to verify. Do not automatically launch the application, operate its UI, or capture its screen unless the user specifically asks for that evidence. Instead, provide a concise manual verification checklist; the user may report a discrepancy with a screenshot. On macOS, when a launch is explicitly requested, run `sh scripts/launch-debug-macos.sh`; it builds the application, creates or updates a temporary `.app` bundle, and opens it through Launch Services without Terminal. If a requested launch cannot overwrite a running instance, ask the user to close it, then retry.

Run build commands from the repository root. Prefer debug builds for quick local verification. Use focused checks during iterations; full build and validation are required before handoff of each completed functional slice, including a slice that repairs a failed build or release.
