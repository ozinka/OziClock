...
use oziclock_storage::{AppSettings, ClockSettings, Stopwatch, StopwatchState};
...
pub(crate) fn run() -> Result<(), slint::PlatformError> {
    ...
    planner_event_bindings::wire_event_bindings(
        alert_sound.clone(),
        &planner_window,
        &event_attention_window,
        &window,
        shared_settings.clone(),
        planner_event_bindings::EventPlanModels::new(
            plan_event_model.clone(),
            plan_all_day_event_model.clone(),
        ),
        plan_week_start.clone(),
    );
