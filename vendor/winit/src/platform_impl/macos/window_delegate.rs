...
impl WindowDelegate {
    ...
    pub(crate) fn is_zoomed(&self) -> bool {
        // Querying state must not add a title bar: AppKit can constrain the
        // temporary titled frame below the menu bar and leave it displaced.
        let window = self.window();
        ...
        // These windows use an explicit work-area frame in set_maximized.
        // Read the actual frame so moving/resizing also reconciles the state.
        window.screen().is_some_and(|screen| window.frame() == screen.visibleFrame())
    }
    ...
    pub fn set_maximized(&self, maximized: bool) {
        ...
        let required = NSWindowStyleMask::Titled | NSWindowStyleMask::Resizable;
        if self.window().styleMask().contains(required) {
            // Native zoom is reliable for titled, resizable windows.
            self.window().zoom(None);
        } else {
            // Borderless and non-resizable windows use explicit frame geometry.
