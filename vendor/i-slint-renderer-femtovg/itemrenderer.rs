...
fn rect_with_radius_to_path(
    ...
    if let Some(border_radius) = border_radius.as_uniform() {
        ...
        } else if border_radius * 2. >= width.min(height) {
            // OziClock MODE-06A: capsules have the same zero-length-edge issue as
            // circles. Join their quarter circles directly at the rounded ends.
            let r = width.min(height) / 2.;
            let c = r * (1. - 0.552_284_8);
            path.move_to(x, y + r);
            if height > 2. * r {
                path.line_to(x, y + height - r);
            }
            path.bezier_to(x, y + height - c, x + c, y + height, x + r, y + height);
            if width > 2. * r {
                path.line_to(x + width - r, y + height);
            }
            path.bezier_to(x + width - c, y + height, x + width, y + height - c, x + width, y + height - r);
            if height > 2. * r {
                path.line_to(x + width, y + r);
            }
            path.bezier_to(x + width, y + c, x + width - c, y, x + width - r, y);
            if width > 2. * r {
                path.line_to(x + r, y);
            }
            path.bezier_to(x + c, y, x, y + c, x, y + r);
            path.close();
        } else {
